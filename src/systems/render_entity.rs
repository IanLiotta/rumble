...
pub fn render_entity(ecs: &SubWorld, commands: &mut CommandBuffer){
    ...
    renderables
        ...
        .for_each(|(entity, pos, render, offset)|{
            ...
            let new_offset_x = if offset.offset_x.abs() >= 0.1 { offset.offset_x - offset.offset_x.signum()*0.1} else {0.0};
            let new_offset_y = if offset.offset_y.abs() >= 0.1 { offset.offset_y - offset.offset_y.signum()*0.1} else {0.0};
            commands.add_component(*entity, DrawOffset{offset_x: new_offset_x,offset_y:new_offset_y})
